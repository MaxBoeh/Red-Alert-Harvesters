...
Harvester = {
	New = function(entity)
		...
		self.animEntity = game.surfaces.nauvis.create_entity{name = "harvester-anim", force=self.vehicle.force, position=self.vehicle.position}
		-- Get rid of the flashing "no fuel" icon.
		self.animEntity.get_inventory(defines.inventory.fuel).insert({name = "coal", count = 1})
		
		self.animationState = HarvesterAnimator.New(self)
	...
	Onload = function(self)
		setmetatable(self, {__index=Harvester})
		HarvesterAnimator.Onload(self.animationState)
	end,
	...
	Tick = function(self)
		...
		st = self.state
		Harvester.StateFunctions[st](self)
	end,
	...
	StateFunctions = {
		...
		[States.DroppingOre] = function(self)
			...
			if self.targetRefinery.entity.valid then
				if Refinery.GetAvailableSlots(self.targetRefinery) > Stats.HarvesterCargoSlots then
